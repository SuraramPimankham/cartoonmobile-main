      ...
      if (_user != null) {
        ...
        if (userSnapshot.exists) {
          ...
          if (sharedWith.containsKey(friendName)) {
            ...
            Map<String, dynamic> friendSgareData = sharedWith[friendName];
            if (!friendSgareData.containsKey(widget.episodeData['id'])) {
              // ถ้าไม่มีข้อมูล episode ของเพื่อน ให้เพิ่มข้อมูลใหม่ลงไป
              friendSgareData[widget.episodeData['id'].toString()] = {
                'episode': {episodeId},
                'expiration':
                    (Timestamp.now().toDate()).add(Duration(minutes: 1)),
              };
            } else {
              // ถ้ามีข้อมูล episode ของเพื่อนคนนี้อยู่แล้ว ให้เพิ่ม episodeId เข้าไปใน list ที่มีอยู่
              List<dynamic> episodes = List.from(
                  friendSgareData[widget.episodeData['id'].toString()]
                      ['episode']);
              episodes.add(episodeId);
              friendSgareData[widget.episodeData['id'].toString()]['episode'] =
                  episodes;
            }
          } else {
            ...
            sharedWith[friendName] = {
              widget.episodeData['id'].toString(): {
                'episode': {episodeId},
                //  'expiration':(Timestamp.now().toDate()).add(Duration(days: 7)),
                'expiration':
                    (Timestamp.now().toDate()).add(Duration(minutes: 1)),
              }
            };
